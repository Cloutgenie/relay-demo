...
export async function seedDemoWorkspace() {
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://relay.vercel.app";
  ...
  const messageCount = await prisma.message.count({ where: { tenantId: tenant.id } });
  if (messageCount > 0) return { email, password, skippedInbox: true };
  ...
  for (const sample of DEMO_SAMPLES) {
    await processInbound(tenant.id, sample);
  }
  ...
  return { email, password, skippedInbox: false };
}
